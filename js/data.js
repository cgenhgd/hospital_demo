...
AjaxRemoteGetData.getLevelAreaSoftByHospitalName=function(name){
	console.log('远程数据获取','getLevelAreaSoftByHospitalName');
	var len=storage.hospital.length;
	for(var i=0;i<len;i++){
		var hospitalName=storage.hospital[i][3];
		if(hospitalName==name){
			return storage.hospital[i];
		}
	}
}
...
AjaxRemoteGetData.getLevelByArea  = function( area ){
	console.log('远程数据获取','getLevelByArea','arguments:',arguments);
